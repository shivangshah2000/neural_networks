...
def stochastic_grad_descent(X , y, n_iter, alpha,theta):
    m=len(y)
    ...
    for it in range(n_iter):
        ...
        for i in range(m):
            ...
            theta=theta- alpha*(X_i.T.dot((predict-y_i)))
            cost_sum+= cost(X_i,y_i,theta)
        cost_history[it]=cost_sum
    return theta,cost_history

def mini_batch_grad_descent(X,y,theta_mini,alpha,n_iter,batch_size=32):
    m=len(y)
    cost_history = np.zeros(n_iter)
    ...
    for it in range(n_iter):
        ...
        for i in range(0,m,batch_size):
            ...
            theta_mini= theta_mini - alpha* (X_i.T.dot((prediction-y_i)))
...
theta , cost_history = stochastic_grad_descent(Xnew , y , n_iter, alpha, theta)
